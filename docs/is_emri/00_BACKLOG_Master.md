...
**Son güncelleme:** 30 Nisan 2026 akşam (v16.26 ADMIN_EMAILS array + token boş string fix · 23 sürüm + 10 DB migration tek günde rekor)
...
| **v16.22 (30 Nis öğleden sonra)** | **Operatör Auth pilot — yan yana yaklaşım** — `Login.doOprLogin` içinde sicil_hash başarılı olunca arka planda `signInWithPassword`. `useAuth` `@uys.local` email'leri için Auth session koruyor. Plus DB: `uys_operators.auth_user_id uuid` kolonu + index. TEST PILOT pilot doğrulandı. |
| **v16.23 (30 Nis öğleden sonra, DB-only)** | **KOD10 ERKİN için Supabase Auth user** (pilot, gerçek operatör) — `op_kod10@uys.local` + 1234 şifre. |
| **v16.24 (30 Nis öğleden sonra)** | **Admin login OPR_KEY temizle fix** — `useAuth` getSession + onAuthStateChange ADMIN_EMAILS branch'lerine `sessionStorage.removeItem(OPR_KEY)`. |
| **v16.25 (30 Nis öğleden sonra, DB-only)** | **RLS Aşama 3 — 89 operatör bulk Supabase Auth migration**. Tek migration ile aktif 89 operatöre `op_<kod_lower>@uys.local` Auth user oluşturuldu. **Aşama 4** denendi → login akışı anon role ile yapıldığı için kırıldı, **rollback** yapıldı. Plus admin Auth user yenileme: çoklu SQL UPDATE state'i bozdu, Buket Dashboard'dan yeniden oluşturdu (UUID `ff76792a...`). **§27.10:** auth.users tablosuna doğrudan DELETE+INSERT yapılmaz. |
| **v16.26 (30 Nis akşam)** | **TOKEN BOŞ STRING FIX + admin@uys.local alternatif** — 89 operatör Auth user'larında token alanları NULL'dan `''`'e çevrildi (signInWithPassword sessizce fail oluyordu, §27.11). KOD83 ile gerçek `last_sign_in_at` 29 saniye önce → **89/89 operatör gerçekten Supabase Auth'lu** doğrulandı. Plus admin için sentetic `admin@uys.local` Auth user (gmail.com rate limit bypass), şifre `1234`. `useAuth.ADMIN_EMAILS` array genişletildi `['uzuniskender@gmail.com', 'admin@uys.local']`. |
