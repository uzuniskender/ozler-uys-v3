...
export const ACTION_GROUPS: { group: string; actions: { key: string; label: string }[] }[] = [
  ...
  { group: 'Depo / Stok', actions: [
    ...
    { key: 'stok_rezerv_ekle', label: 'Stok rezervasyonu ekleme (admin)' },
    { key: 'stok_rezerv_iptal', label: 'Stok rezervasyonu iptal (admin)' },
    { key: 'stok_rezerv_goruntule', label: 'Stok rezervasyonu görüntüleme' },
  ]},
...
export const DEFAULTS: Record<string, AdminRole[]> = {
  ...
  stok_rezerv_ekle:[],        // admin only
  stok_rezerv_iptal:[],       // admin only
  stok_rezerv_goruntule:['planlama','uretim_sor'],
