...
// Hem local kullanım (line 63/116/179) hem dış re-export için import + export
import { getStok, getYolda } from '@/lib/hammaddeHesap'
export { getStok, getYolda }
...
export async function siparisRevizeUygula(
  ...
  for (const kd of delta.kalemDeltalari) {
    ...
    if (kd.tip === 'kalem_ekle') {
      ...
      const { buildWorkOrders } = await import('@/services/productionService/autoChain')
