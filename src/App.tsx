...
export default function App() {
  ...
  // v16.36 — Multi-device single-session: başka cihazdan giriş yapıldıysa
  // 5 saniyelik geri sayım modalı göster, sonra otomatik logout.
  // Operatör: kayıt edilmemiş üretim varsa kaybeder → mesajda uyarıyoruz.
  ...
  useEffect(() => {
    ...
    const tick = setInterval(() => {
      setSessionCountdown(prev => {
        if (prev <= 1) {
          clearInterval(tick)
          // İçinde sayfa reload var; setTimeout(0) ile state update'i flush et
  ...
  if (isOperator) {
    return (
      <>
        ...
        {/* v16.36 — Session invalidated modal (operatör için de) */}
        {sessionInvalidated && (
              ...
              <div className="flex items-start gap-3 mb-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-zinc-300 leading-relaxed">
                    Hesabınız <span className="font-semibold text-zinc-100">{sessionInvalidated.deviceLabel}</span> üzerinden
                    açıldı. Bu cihaz <span className="font-bold text-red">{sessionCountdown}</span> saniye içinde otomatik
                    olarak çıkış yapacak.
                  </p>
                  <p className="text-xs text-amber mt-2">
                    Yarım kalan üretim girişi varsa hızlıca kaydedin.
                  </p>
                </div>
              </div>
              <button
                onClick={signOut}
                className="w-full py-2.5 bg-red/20 hover:bg-red/30 border border-red/40 text-red rounded-lg text-sm font-semibold transition-colors"
              >
                Hemen Çıkış Yap
              </button>
            </div>
  ...
  return (
    <>
      <Toaster theme="dark" position="bottom-right" richColors closeButton />
      {/* v16.36 — Session invalidated modal (tüm rollerde geçerli, en üstte render) */}
      {sessionInvalidated && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 backdrop-blur-sm">
          <div className="bg-bg-1 border-2 border-red rounded-2xl p-6 max-w-md mx-4 shadow-2xl">
            <div className="flex items-start gap-3 mb-4">
              ...
              <div className="flex-1">
                ...
                {(role === 'operator' || role === 'admin') && (
                  <p className="text-xs text-amber mt-2">
                    Kaydedilmemiş bilgileriniz varsa hızlıca kaydedin.
                  </p>
                )}
              </div>
            </div>
