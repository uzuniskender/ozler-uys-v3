...
export function Operations() {
  ...
  async function save(kod: string, ad: string, editId?: string) {
    if (editId) await supabase.from('uys_operations').update({ kod, ad }).eq('id', editId)
    else await supabase.from('uys_operations').insert({ id: uid(), kod, ad })
    ...
    <div>
      ...
      <div className="bg-bg-2 border border-border rounded-lg overflow-hidden">
        <table className="w-full text-xs">
          <thead><tr className="border-b border-border text-zinc-500"><th className="text-left px-4 py-2.5">Kod</th><th className="text-left px-4 py-2.5">Operasyon Adı</th><th className="px-4 py-2.5"></th></tr></thead>
          <tbody>
            {filtered.map(o => (
...
function SimpleFormModal({ title, initial, onClose, onSave }: { title: string; initial: { id: string; kod: string; ad: string } | null; onClose: () => void; onSave: (kod: string, ad: string, id?: string) => void }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div className="bg-bg-1 border border-border rounded-xl p-6 w-full max-w-sm" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex justify-end gap-2 mt-5">
          ...
          <button onClick={() => onSave(kod, ad, initial?.id)} className="px-4 py-2 bg-accent hover:bg-accent-hover text-white rounded-lg text-xs font-semibold">Kaydet</button>
        </div>
      </div>
    </div>
  )
