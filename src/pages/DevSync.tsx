...
// Sadece bu uzantılar sync edilir
...
export function DevSync() {
  ...
  // Supabase'deki mevcut dosyaları yükle
  async function loadSynced() {
    const { data } = await supabase
      ...
      .order('path')
    setSyncedFiles(data || [])
    // Claude'un değiştirdiği dosyalar (updated_by = 'claude')
    const changes = (data || []).filter(f => f.updated_by === 'claude')
    setPendingChanges(changes)
  }

  // GitHub'dan repo ağacını çek
  async function loadRepoTree() {
    ...
    try {
      ...
      setRepoFiles(files)
      // Varsayılan seçim: src/ altındaki tüm dosyalar
  ...
  // Seçili dosyaları Supabase'e sync et
  async function syncSelected() {
    ...
    await loadSynced()
    setSyncing(false)
    toast.success(`${ok} dosya senkronize edildi`)
  }

  // Claude'un değiştirdiği dosyayı indir
        ...
        <div className="bg-bg-2 border border-border rounded-xl overflow-hidden">
          ...
          <div className="flex items-center gap-3 px-4 py-3 border-b border-border">
            ...
            <button onClick={() => setSelectedPaths(new Set(repoFiles.map(f => f.path)))}
              className="px-2 py-1 bg-bg-3 text-zinc-400 rounded text-[10px] hover:text-white">Tümünü Seç</button>
            <button onClick={() => setSelectedPaths(new Set(repoFiles.filter(f => f.path.startsWith('src/')).map(f => f.path)))}
              className="px-2 py-1 bg-bg-3 text-zinc-400 rounded text-[10px] hover:text-white">src/ Seç</button>
          ...
          <div className="overflow-y-auto max-h-[60vh]">
            {loading && <div className="p-8 text-center text-sm text-zinc-500">⟳ GitHub'dan yükleniyor...</div>}
                ...
                {files.map(f => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {synced && !isChanged && <span className="px-1.5 py-0.5 bg-green/10 text-green rounded text-[9px] shrink-0">✓</span>}
