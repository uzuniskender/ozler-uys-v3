...
$tumu = Get-ChildItem -Path $kaynak -Recurse -File -Include *.pdf,*.dxf,*.dwg,*.stp,*.step -ErrorAction SilentlyContinue
Write-Host "$($tumu.Count) dosya tarandi (pdf/dxf/dwg/stp/step)."
...
`}async function A(){const{data:e,error:n}=await l.from("uys_teknik_resim_envanter_tarama").select("ts").order("ts",{ascending:!1}).limit(1);if(n||!e||e.length===0)return{tarih:null,yasGun:null};const t=String(e[0].ts);return{tarih:t,yasGun:Math.round((Date.now()-new Date(t).getTime())/864e5)}}async function f(e,n){const t=e.map(r=>r.trim()).filter(r=>/\.pdf$/i.test(r));if(t.length===0)throw new Error("Listede .pdf ile biten satır yok");const{data:a,error:o}=await l.rpc("uys_teknik_resim_envanter_yaz",{p_dosyalar:t,p_kullanici:n??null});if(o)throw o;const i=Array.isArray(a)?a[0]:a;return{dosya:Number(i?.r_dosya??0),kod:Number(i?.r_kod??0),yeni:Number(i?.r_yeni??0),kaybolan:Number(i?.r_kaybolan??0)}}function h(e=m){return`# UYS — Ortak alan envanteri. Sadece DOSYA ADLARINI panoya kopyalar (dosya kopyalanmaz).
