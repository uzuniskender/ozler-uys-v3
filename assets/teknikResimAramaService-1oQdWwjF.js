import{r as l}from"./supabase-BYHBnSSI.js";var y={VAR:"bg-emerald-400 text-black",KARDES_AYNI_GRUP:"bg-amber-400 text-black",KARDES_FARKLI_GRUP:"bg-orange-500 text-black",YOK:"bg-zinc-700 text-zinc-300",SORULAMADI:"bg-zinc-800 text-zinc-300 border border-dashed border-zinc-500"},d={VAR:"Resim var",KARDES_AYNI_GRUP:"Kardeşte var",KARDES_FARKLI_GRUP:"Kardeşte var (farklı grup)",YOK:"Resim yok",SORULAMADI:"Sorulamadı (kartı yok)"};function k(a){return a==="VAR"?"VAR":a&&a.startsWith("KARDEŞTE VAR (aynı")?"KARDES_AYNI_GRUP":a&&a.startsWith("KARDEŞTE VAR")?"KARDES_FARKLI_GRUP":"YOK"}async function A(a){const n=new Map,t=Array.from(new Set(a.map(r=>(r||"").trim()).filter(Boolean)));if(t.length===0)return n;for(const r of t)n.set(r,{kod:r,ad:null,durum:"SORULAMADI",kardesKod:null,etiket:d.SORULAMADI,analiz:"SORULAMADI",analizKardes:null});for(let r=0;r<t.length;r+=200){const{data:i,error:o}=await l.from("uys_teknik_resim_durum_v").select("kod, ad, ortak_alan_durum, ortak_alan_kardes, analiz_durum, analiz_kardes").in("kod",t.slice(r,r+200));if(o)throw o;for(const e of i??[]){const s=String(e.kod),u=k(e.ortak_alan_durum==null?null:String(e.ortak_alan_durum));n.set(s,{kod:s,ad:e.ad==null?null:String(e.ad),durum:u,kardesKod:e.ortak_alan_kardes==null?null:String(e.ortak_alan_kardes),etiket:d[u],analiz:k(e.analiz_durum==null?null:String(e.analiz_durum)),analizKardes:e.analiz_kardes==null?null:String(e.analiz_kardes)})}}return n}var c="O:\\ARGE\\TEKNIK_RESIM";function f(a,n=c,t=["pdf","dxf","dwg","stp","step"]){const r=Array.from(new Set(a.map(e=>(e||"").trim()).filter(Boolean))),i=r.map(e=>'"'+e+'"').join(","),o=(t.length?t:["pdf"]).map(e=>"*."+e.replace(/^\./,"")).join(",");return`# UYS — Teknik Resim Toplayici (${r.length} kod)
...
`}async function R(){const{data:a,error:n}=await l.from("uys_teknik_resim_envanter_tarama").select("ts").order("ts",{ascending:!1}).limit(1);if(n||!a||a.length===0)return{tarih:null,yasGun:null};const t=String(a[0].ts);return{tarih:t,yasGun:Math.round((Date.now()-new Date(t).getTime())/864e5)}}var m=["pdf","dwg","dxf","stp","step"],$=/\.(pdf|dwg|dxf|stp|step)$/i;async function h(a,n){const t=a.map(e=>e.trim()).filter(e=>$.test(e));if(t.length===0)throw new Error("Listede çizim dosyası yok (beklenen uzantılar: "+m.join(", ")+")");const{data:r,error:i}=await l.rpc("uys_teknik_resim_envanter_yaz",{p_dosyalar:t,p_kullanici:n??null});if(i)throw i;const o=Array.isArray(r)?r[0]:r;return{dosya:Number(o?.r_dosya??0),kod:Number(o?.r_kod??0),yeni:Number(o?.r_yeni??0),kaybolan:Number(o?.r_kaybolan??0)}}function p(a=c){const n=m.map(t=>"*."+t).join(",");return`# UYS — Ortak alan envanteri. Sadece DOSYA ADLARINI panoya kopyalar (dosya kopyalanmaz).
# ${n} taranir — delik verisi DWG/DXF'ten cikar, PDF cogu zaman sadece resimdir.
Get-ChildItem "${a}" -Recurse -File -Include ${n} |
...
`}export{h as a,p as i,y as n,f as o,R as r,A as s,d as t};
